...
- **飞书**：WebSocket 长连接收消息，Open API 发送；接收侧支持文本 / 图片 / 文件 / 音视频（按消息类型和资源键下载）；群聊时在消息 metadata 中带 `feishu_chat_id`、`feishu_message_id` 便于下游去重与群上下文。
- **Discord**：接收时附件会解析为图片 / 视频 / 音频 / 文件并传入 Agent；回复时真实附件发送为 🚧 施工中，当前仅以链接形式附在文本中。
- **Telegram**：接收侧支持文本与附件，附件会先把 `file_id` 解析为可下载 URL 再进入 Agent；回复时真实附件发送为 🚧 施工中，当前仅以链接形式附在文本中。
- **iMessage**：基于本地 imsg + 数据库轮询，仅支持文本收发；平台/实现限制，无法支持附件（✗）。
- **QQ**：接收侧支持把附件解析为图片 / 视频 / 音频 / 文件并传入 Agent；发送侧真实媒体仍为 🚧 施工中，当前仅文本 + 链接形式。
